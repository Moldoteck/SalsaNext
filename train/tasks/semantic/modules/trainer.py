...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None):
        ...
        #                                 lr=self.ARCH["train"]["lr"],
        #                                 weight_decay=self.ARCH["train"]["w_decay"])
        # self.optimizer = deepspeed.ops.adam.DeepSpeedCPUAdam([{'params': self.model.parameters()}],
        #                                                      lr=self.ARCH["train"]["lr"], bias_correction=True, 
        #                                                      betas=(0.9, 0.999), eps=1e-08, 
        #                                                      weight_decay=self.ARCH["train"]["w_decay"], 
        #                                                      amsgrad=False, adamw_mode=True)
        ...
        # self.scheduler = deepspeed.runtime.lr_schedules.WarmupDecayLR(
        ...
        deepspeed.checkpointing.configure(
            None, deepspeed_config='./modules/ds_config.json')
        ...
        for epoch in range(self.epoch, self.ARCH["train"]["max_epochs"]):
            ...
            if self.info['train_iou'] > self.info['best_train_iou']:
                ...
                self.model.save_checkpoint(
                    self.log, "_train_best", client_state=state)
            ...
            if self.info['valid_iou'] > self.info['best_val_iou']:
                ...
                self.model.save_checkpoint(
                    self.log, "_valid_best", client_state=state)
