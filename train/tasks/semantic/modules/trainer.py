...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None):
        ...
        self.model, self.optimizer,_,self.scheduler = deepspeed.initialize(model=self.model, optimizer=self.optimizer,lr_scheduler=self.scheduler,model_parameters=[{'params': self.model.parameters()}], config='./modules/ds_config.json')
